...
Build and packaging release. No API changed and no library code changed. The library's **consumer ProGuard rules** are the entire subject: they were pinning several thousand classes as un-shrinkable in every app that depends on AdManageKit, and they are now empty. Apps that run R8 get the win by upgrading; nothing needs to be configured.

### Changed

- **`ad-manage-kit-compose` no longer pins the whole Compose runtime in consuming apps.** A consumer rule is not advice — it is appended to the R8 configuration of every app that depends on the module, and the app cannot opt out of it. This module shipped `-keep class androidx.compose.** { *; }`, which R8 reads as `DONT_SHRINK, DONT_OPTIMIZE, DONT_OBFUSCATE` applied to **7221 classes, 57262 methods and 23964 fields** — the Compose runtime, foundation, UI and Material3 in their entirety, retained whether the app touches them or not, and retained *even for apps that never call a single AdManageKit composable*. It also defeated the `allowshrinking` / `allowoptimization` latitude that Compose's own consumer rules deliberately grant R8. Two further rules in the same file, `-keep class com.google.android.gms.ads.** { *; }` (97 classes / 506 methods) and `-keep class com.i2hammad.admanagekit.compose.** { *; }` (47 classes / 446 methods), were removed for the same reason. None of the three was load-bearing: the module's composables are ordinary Kotlin functions that reach the view layer through `AndroidView` factory lambdas, and the module contains no reflection, no `Class.forName`, no resource-name lookup and no serialization, so R8 already sees every edge it needs from the consuming app's own call sites
- **`consumer-rules.pro` is now present, wired and deliberately empty in all five modules.** `ad-manage-kit-core`, `ad-manage-kit-billing` and `ad-manage-kit-yandex` previously declared no `consumerProguardFiles` at all, and `ad-manage-kit`'s file existed but was blank — so the contract was accidental rather than stated. Each file now carries the reasoning for why it holds no rules, so the next person to hit an R8 problem has something to read instead of a blank file to guess at. No module in the library uses reflection, `Class.forName`, JNI, `Parcelable`, `Serializable` or reflective serialization. The ad views (`BannerAdView`, `NativeBannerSmall`/`Medium`, `NativeLarge`, `NativeTemplateView`) need no keep rule either: AAPT2 already emits `-keep class <view> { <init>(android.content.Context, android.util.AttributeSet); }` into the generated `aapt_rules.txt` for exactly the views each app's own layouts reference, which is strictly better than keeping all of them in all apps. The 38 native templates are referenced statically as `R.layout.*` constants from `NativeAdTemplate`, so both R8 and the resource shrinker keep them along with the `ad_headline` / `ad_media` / `ad_body` / `ad_call_to_action` / `ad_app_icon` ids that `YandexNativeProvider` resolves by name through `Resources.getIdentifier`
- **The sample app now builds release with `isMinifyEnabled` and `isShrinkResources` on.** Both were off, which is why none of the above was ever noticed: R8 had never run anywhere in this repository, so the rules shipped to consumers were never once exercised by the project that ships them. They now are, on every release build. On the sample app — which depends on all five modules, Yandex included — the effect of this release is 11.99 MB → **7.89 MB** of APK (−34%), 21.3 MB of uncompressed dex across 3 files → **12.5 MB across 2** (−41%), 37713 → **29297** live classes, 186338 → **131255** live methods, and 19307 → **11856** classes pinned by keep rules
- **`android.r8.strictFullModeForKeepRules` and `android.r8.optimizedResourceShrinking` are back at their AGP 9 defaults.** Both were set to `false` in v3.5.6, the release that moved the project from AGP 8.13.1 to 9.0.1, described in that commit as being "for improved build stability during the transition". Nothing ever exercised them, because minification was off, so the transition they were guarding was already over before R8 ran for the first time. Strict full mode stops a member-less `-keep class X` from silently pinning X's members; optimized resource shrinking lets unreachable resources actually go

### Notes

- **Nothing to migrate, and no action required in host apps.** An app that already enables R8 simply builds smaller after the upgrade. An app that carries its own defensive `-keep class com.i2hammad.admanagekit.** { *; }` can now delete it — the library never needed it — but leaving it in place is harmless beyond the size it costs
- **Verification boundary.** The removals are backed by a full R8 configuration-analyzer pass (no keep rule in the minified sample app now traces back to a project file — only AGP's own `proguard-android-optimize.txt` defaults), by confirming the shrunk `resources.arsc` still carries every native template layout and every dynamically-resolved `ad_*` id, and by the AAPT2-generated view constructor keeps being present as described. The existing 176 tests pass unchanged, but they run on the unminified debug variant and therefore do not cover R8 output; on-device verification of a minified release build is recommended, particularly for Yandex native templates, which are the only place in the library that resolves resources by name
- **The `ad-manage-kit-yandex` dependency remains the dominant cost in a minified build**, and that cost is not the library's to remove: the Yandex Mobile Ads SDK's own consumer rules pin 5788 distinct classes (5631 of them under `-keepnames class yads.**` alone), and the AppMetrica tree it pulls in pins a further 2297. Apps that use only the AdMob providers avoid all of it by not depending on the module
- The five library modules continue to publish **unminified** AARs (`isMinifyEnabled = false`), deliberately — a published AAR must keep its public API under its real names or consumers cannot compile against it, and shrinking there would only remove code the consuming app's R8 removes anyway, with far better whole-program information. Each module's `proguard-rules.pro` now says so. The one in `ad-manage-kit` previously ended with `-keep class * { *; }`, inert while minification was off but a rule that would have disabled R8 for the entire program had anyone switched it on
