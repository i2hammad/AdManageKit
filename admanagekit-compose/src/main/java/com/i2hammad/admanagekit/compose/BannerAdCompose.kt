...
fun BannerAdCompose(
    ...
    // Reserve the real ad height up front so the layout doesn't jump on load:
    // fixed sizes know their height, adaptive banners are 50-90dp tall
    // depending on the device, so a hardcoded 50dp box would clip them
