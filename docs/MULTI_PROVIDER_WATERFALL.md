...
class MyApp : Application() {
    ...
    private fun configureMultiProvider() {
        ...
        AdUnitMapping.register("ca-app-pub-xxx/app-open", mapOf(
            "yandex" to "R-M-XXXXXX-Y"
        ))
        AdUnitMapping.register("ca-app-pub-xxx/rewarded", mapOf(
            "yandex" to "R-M-XXXXXX-Y"
        ))
...
```kotlin
// Inside AdManager, BannerAdView, NativeBannerSmall, etc.
...
fun loadInterstitialAd(context: Context, adUnitId: String) {
    ...
    // ... existing AdMob code completely unchanged ...
}
```

### Native Ad Sizes

Native ad providers receive a `NativeAdSize` hint (`SMALL`, `MEDIUM`, `LARGE`) so they can return appropriately sized views:

| Consumer View | Size Hint | Yandex Rendering |
|---------------|-----------|-----------------|
| `NativeBannerSmall` | `SMALL` | Icon + title + CTA (single row) |
| `NativeBannerMedium` | `MEDIUM` | Icon + title + body + CTA (no media) |
| `NativeLarge` | `LARGE` | Full `NativeBannerView` template (with media) |
| `NativeTemplateView` | Auto | Based on selected template category |
...
private fun configureMultiProvider() {
    ...
    if (isRussia) {
        ...
        // AdMob first for everywhere else
        AdProviderConfig.setInterstitialChain(listOf(admob.interstitialProvider, yandex.interstitialProvider))
        AdProviderConfig.setNativeChain(listOf(admob.nativeProvider, yandex.nativeProvider))
    }
}
